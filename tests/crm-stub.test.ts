...
describe('CrmStub', () => {
  ...
  it('returns a seeded account by externalCrmId', async () => {
    ...
    expect(got?.currentArc).toBe(600000);
  });

  it('listAccountsModifiedSince returns only accounts with mtime >= cutoff', async () => {
    const crm = new CrmStub();
    crm.seed({ externalCrmId: 'OLD', clientName: 'Old', currentArc: 12000, contractStatus: 'ACTIVE', onboardingDate: new Date('2024-01-01'), modifiedAt: new Date('2026-01-01') });
    crm.seed({ externalCrmId: 'NEW', clientName: 'New', currentArc: 24000, contractStatus: 'ACTIVE', onboardingDate: new Date('2026-04-15'), modifiedAt: new Date('2026-04-20') });
